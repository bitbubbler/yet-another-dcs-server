...
export class Migration20230106034759 extends Migration {
  ...
  async up(): Promise<void> {
    ...
    this.addSql('create unique index `markups_positionId_unique` on `markups` (`positionId`);');
    this.addSql('create index `markups_type_index` on `markups` (`type`);');

    this.addSql('create table `markupPoints` (`markupId` integer not null, `positionId` integer not null, `index` integer not null, constraint `markupPoints_markupId_foreign` foreign key(`markupId`) references `markups`(`markupId`) on update cascade, constraint `markupPoints_positionId_foreign` foreign key(`positionId`) references `positions`(`positionId`) on update cascade, primary key (`markupId`, `positionId`));');
    this.addSql('create index `markupPoints_markupId_index` on `markupPoints` (`markupId`);');
    this.addSql('create unique index `markupPoints_positionId_unique` on `markupPoints` (`positionId`);');
    ...
    this.addSql('create unique index `units_positionId_unique` on `units` (`positionId`);');
